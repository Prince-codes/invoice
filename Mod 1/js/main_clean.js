...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildTable(start, end) {
    ...
    container.className = 'daily-table';
    ...
    thead.innerHTML = '<tr><th>S.no</th><th>Date</th><th>Amount</th></tr>';
  ...
  function getNextInvoiceId() {
    const current = Number(localStorage.getItem('nextInvoiceId') || '1');
    localStorage.setItem('nextInvoiceId', String(current + 1));
    return current;
  }
  ...
  function openPrintWindow(inv) {
    const daily = Array.isArray(inv.daily) ? inv.daily.map(r => ({ ...r })) : [];
...
<head>
  ...
  <style>
    ...
    body { margin: 0; padding: 0; font-family: 'Montserrat', 'Segoe UI', Arial, sans-serif; color: #111827; background: #f8fafc; font-size: 13px; }
    .invoice-shell { width: calc(100% - 9mm); max-width: 201mm; margin: 0 auto; background: #fff; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.08); padding: 18px 22px 24px; border-radius: 14px; box-sizing: border-box; page-break-after: always; }
    .invoice-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; padding-bottom: 12px; border-bottom: 2px solid #cbd5e1; flex-wrap: wrap; }
    .brand-block { display: flex; align-items: center; gap: 12px; }
    .logo-wrap { width: 68px; height: 68px; display: flex; align-items: center; justify-content: center; border-radius: 14px; background: linear-gradient(135deg, #f59e0b, #fcd34d); box-shadow: 0 6px 20px rgba(245, 158, 11, 0.16); }
    ...
    .meta-pill { margin-bottom: 6px; padding: 5px 8px; border-radius: 999px; background: #f3f4f6; color: #374151; font-size: 11px; display: inline-block; border: 1px solid #cbd5e1; }
    .bill-to { display: flex; justify-content: space-between; align-items: center; margin: 16px 0 12px; padding: 10px 12px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 10px; }
    ...
    .status-tag { background: #ecfeff; color: #0f766e; padding: 6px 10px; border-radius: 999px; font-size: 11px; font-weight: 700; }
    .table-grid { display: flex; gap: 12px; margin-top: 8px; flex-wrap: wrap; }
    .table-card { flex: 1 1 48%; min-width: 260px; background: #fff; border: 1px solid #cbd5e1; border-radius: 10px; overflow: hidden; page-break-inside: avoid; }
    .table-card h3 { margin: 0; padding: 8px 10px; font-size: 13px; background: #f9fafb; color: #374151; border-bottom: 1px solid #cbd5e1; }
    ...
    .summary-card { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; margin-top: 16px; padding: 12px 14px; background: linear-gradient(135deg, #f8fafc, #f3f4f6); border: 1px solid #cbd5e1; border-radius: 12px; page-break-inside: avoid; }
    .summary-list { flex: 1; }
    .summary-row { display: flex; justify-content: space-between; padding: 5px 0; color: #374151; font-size: 12px; }
    .summary-row.total { margin-top: 6px; padding-top: 8px; border-top: 1px dashed #d1d5db; font-size: 14px; font-weight: 700; color: #111827; }
    .words-block { min-width: 230px; padding-left: 16px; border-left: 2px solid #d1d5db; }
    ...
    @media print {
      ...
      .invoice-header, .table-grid, .summary-card, .bill-to { page-break-inside: avoid; }
    ...
    viewPreviousBtn.addEventListener('click', () => {
      ...
      invoices.forEach(inv => {
        ...
        row.innerHTML = `
          ...
          <td style="text-align:center;">${inv.monthText || inv.monthYearISO}</td>
  ...
  function buildInvoiceData() {
    ...
    dailyRows.forEach(r => {
      if (!r.is_empty && r.amount !== null) {
        if (r.day_num <= 15) t1 += Number(r.amount);
        else t2 += Number(r.amount);
      }
    });
